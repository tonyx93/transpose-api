...
def simple():
    ...
    Notes:
    If a vector is provided, it will be transposed into a 1 x n dimension matrix where n is the dimensions of the original vector
    In this scenario, this transposition will not be invertible, meaning passing the output vector will not get your the original vector.
    Instead, you will get a n x 1 dimension matrix.
    e.g. [1,2,3] -> [[1],[2],[3]] -> [[1, 2, 3]] -> [[1],[2],[3]]

    This operation will truncate rows to the match the length of the shortest row if the rows are uneven i.e. bad input
    ...
    """
    matrix = request.json
    return list(zip(*matrix)) if isinstance(matrix[0], list) else [[v] for v in matrix]
...
def raw():
    """
    Transposing a matrix using string/byte manipulation
            ...
            for i in range(len(index)):
                ...
                match data[ptr]:
                    ...
                    case _:
                        ...
                        raise ValueError(
                            f"Unexpected '{data[ptr]}' character encountered at position {ptr}"
                        )
            yield "]"
            if not last:
                yield ","
    ...
    # Returning a generator in Flask response sets Transfer-Encoding: chunked. Since we already know the content length of the response, we should not use this.
    # return Response(generate(), mimetype="application/json")
    return Response("".join(generate()), mimetype="application/json")
